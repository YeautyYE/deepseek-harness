...
浏览历史 Session 会产生四类独立的解码数据所有者：可复用的 Host observation 缓存、持久化交接 memo、实时 follow 流的首屏读取，以及浏览器中单个聊天节点的数据源。显示页较小并不限制这些所有者保留的对象。条目数限制仍允许日志无限增大；挂起的生成器可能保留已完成首屏读取的局部变量；强引用的按键数据源表会在窗口替换后继续保留节点。
...
[JSONL 交接 memo](../../../../packages/session/session-persistence-jsonl/src/index.ts) 至多保存两个已解析日志的弱引用。查找可以复用同一修订中仍可达的已校验日志，但 memo 自身不保持解码历史存活。活动句柄及准备与发布操作各自保留强引用。条目回收后按相同校验与不可变代际规则重新解码。

[历史控制器](../../../../packages/api/session-controller/src/history.ts) 把首屏读取委托给短生命周期生成器。该生成器拥有 observation、完整事件数组、分页构建和 snapshot 产出。在首屏生成器完成后，实时 follower 只保留 cursor 和 assistant-stream 切面。监听器安装和缓冲仍然先于 observation，维持 snapshot 在先且事件连续的交付顺序。
...
| JSONL 后端完成两次读取并关闭句柄，每份含 2,048 条消息、每条 16 KiB 文本 | 65.64 MiB；两份日志 | 0.36 MiB；零份日志 |
...
JSONL 测量对每个实现使用三个独立普通 Node 进程，并在后端仍可达时跨事件循环轮次执行五次完整回收；每次运行都会重新打开并校验已回收历史。原实现未通过零保留日志检查。Observation 与 JSONL 测量使用 macOS arm64 / Node 24.5.0，以三次样本的保留堆中位数比较。移除字节限制后恢复到 41.17 MiB。原聊天缓存超过诊断中的 4 MiB 检查值；替换实现保留数据源身份，并为持有对象和仅有订阅的读取方保留全部 40 次通知。包内[持久化 memo](../../../../packages/session/session-persistence-jsonl/tests/cold-log-memo.perf.ts)、[缓存](../../../../packages/session-query/session-query/tests/prepared-cache.perf.ts)和 [follow](../../../../packages/api/session-controller/tests/history-retention.perf.ts) 诊断保留合成复现路径；fixture 不含私人 Session 内容。功能测试覆盖字节淘汰、租约释放、替换、投影失败、首屏取消、已回收 memo 的重新读取、数据源复用和通知所有权。

## 已考虑的替代方案

**把缓存缩减为一个条目。** 单个解码历史仍可能任意增大。数量和字节双重限制既保留有用的小历史，也按体积限制已释放数据的保留量。

**保留两个强引用的持久化 memo 条目。** 条目数限制不约束解码字节数。弱引用查找允许即时复用，无需在查询缓存之外增加另一个长期所有者。回收可能使显式恢复前发生另一次解析。
