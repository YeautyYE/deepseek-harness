...
Every owner disposes its observation. `retain()` creates an independently disposable lease over the same cut for overlapping readers. A live Session that appears during cold resolution wins before publication; a disappeared live source is retried as cold.
...
The observation owns no mutation authority. Its event array is an immutable prefix, and its prepared Session remains unpublished. Explicit Agent commands resolve activation independently; history readers leave the prepared Session unpublished.
...
`session.follow` publishes a required opening snapshot containing header, cursor, the initial event window, and a complete projection baseline. Reconnect replaces the previous generation from another complete snapshot. `session.page` is reserved for older-history reads and gap repair. Observation-only reads, including ordinary follow, never activate an Agent. [Non-activating history follow](../bug-fix/2026-09-20-nonactivating-history-follow.md) supersedes the former promotion policy while retaining the observation and projection decisions.
...
| `session.follow` | One exact observation | All, carried in the opening snapshot | Never |
...
Session consumers share one live-preferred read model and one prepared cold object. Header, events, cursor, and projections belong to the same observation, and repeated reads can reuse a retained preparation. New point-read consumers use SessionQuery instead of composing persistence and registry calls themselves.
