...
A successful v1 `assistant/message` must cite its complete ordered attempt. The migration removes the cited top-level chunks and obsolete message chunk references, compacts the chunks without joining token boundaries, and stores the stream on that message. An unclaimed attempt becomes one log-only `assistant/attempt` at its final chunk position. Unrelated interleaved events keep their relative order. Released packed Tool-call runs with empty identifiers or names become scalar stream chunks, preserving every argument fragment and timestamp; current compact runs retain their non-empty-field rules. A committed empty-id call receives an identifier derived from its turn, step, message sequence, and content index across the stream, message, invocation, and result. The migration requires one matching stream block-end and a fully paired invocation and result in the same step; concurrent empty ids are ambiguous and rejected. Empty names retain the recorded unknown-tool result, and predecessor files remain unchanged.
