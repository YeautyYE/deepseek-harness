...
支持悬停的设备上，较早的用户与 steering 消息操作在悬停或键盘聚焦时出现，最新输入的操作始终可见。CSS 按筛选后的兄弟位置选择较早输入，避免为每行扫描全部后续消息；仅触控的设备保持所有操作行可见。
...
按节点键索引的观察对象在读者持有期间保持身份稳定，历史窗口替换也不例外。订阅会保留观察对象，直到最后一个监听器取消订阅；缓存允许既无订阅又无外部引用的观察对象被回收。
...
- **已加载的 Chat 行保持挂载**——显式逐页展开大型历史会增加 DOM 规模与重新挂载成本；数据源缓存回收并不使 transcript 虚拟化。
