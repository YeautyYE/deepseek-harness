...
注册独立的 `connection.rpc.handle` 通道时，消费者只需注入 `connection`。每个通道等待 WebServer 就绪，在该服务更换后重新挂载，并在注册调用方或返回的 disposer 被释放时撤回；等待 disposer 完成即等待路由移除。即使 WebServer 尚不可用，重复的通道名也会被拒绝。
