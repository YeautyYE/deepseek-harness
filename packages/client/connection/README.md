...
Dedicated `connection.rpc.handle` channels require consumers to inject only `connection`. Each channel waits for its WebServer, remounts when that service is replaced, and withdraws when the registering caller or returned disposer is released; awaiting the disposer waits for route removal. Duplicate channel names are rejected even while no WebServer is available.
