...
WebSocket compression is enabled by default through `websocketCompression`. Compatible peers negotiate `permessage-deflate` without dictionary reuse between messages in either direction; the `ws` library's default threshold leaves small outgoing messages uncompressed. Setting the field to `false`, or connecting a peer that does not offer compression, preserves uncompressed transport. Compression reduces transferred bytes but leaves the decoded JSON, history retention, and plugin payloads unchanged.
