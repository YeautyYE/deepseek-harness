...
`websocketCompression` 默认启用 WebSocket 压缩。兼容的对端协商 `permessage-deflate`，双向均不跨消息复用压缩字典；`ws` 库的默认阈值让较小的出站消息保持未压缩。将该字段设为 `false`，或连接未请求压缩的对端时，传输保持未压缩。压缩减少传输字节，但不改变解码后的 JSON、历史保留方式或插件载荷。
